...
const Trending = () => {
         ...
         // UseEffect for products
          useEffect(() => {
            fetch('/PRODUCTS.json')
              ...
              .then(data => {
                const products = data.filter(item => item.trending);
    ...
    // Page Header
  const TrendingHeader = ({title, image_url}) => {
    return (
      <div className='trending-header' style={{backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.5)), url(${image_url})`}}>
        <h1>{title}</h1>
        </div>
    )
  };
    ...
    const TrendingProducts = () => {
        ...
        return (
          <div className='trending-products'>
            <h2 className='subheading'>Shop Trending</h2>
